...
public class SynchronnizedTriremeScriptEngine {
	...
	public SynchronnizedTriremeScriptEngine(String pathToScripts)
			throws NodeException, InterruptedException, ExecutionException {
		environment = new NodeEnvironment();
		
//		environment.setDefaultNodeVersion(NODE_VERSION);
	...
	private String getResponse(String... args) throws InterruptedException, NodeException, ExecutionException {
		...
		Function callback = new org.mozilla.javascript.BaseFunction() {
			...
			public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
				...
				// hasResult.set(true);
				 System.out.println("response "+args[1]);
		...
		runtime.enqueueCallback(getResponse, moduleResult, moduleResult, runtime.getDomain(),parameters);
//		ScriptFuture x = runtime.getFuture();
//		ScriptStatus status = ScriptStatus.OK;

//		try {
//			ScriptFuture future = reference.getNodeModules();
		ScriptFuture future = runtime.getFuture();	
		future.setListener(new ScriptStatusListener() {

				@Override
				public void onComplete(NodeScript script, ScriptStatus status) 
				{
					System.out.println("Status inside listener is: " + status.getExitCode());
					// status = runtime.getFuture().get(5, TimeUnit.SECONDS);
					// while (!hasResult.get()) {
					// Thread.sleep(10);
					// }
//					assertTrue(true);
					System.out.println("STATUS ::::: "+status.isCancelled());
					if (status.isCancelled()) {
						String response = result.has("response") ? result.getString("response") : null;

						// sb.append(
						// "Method execution and callback time: " +
						// (System.nanoTime() - startTime) / 1000000 + " ms.
						// \n");
						// sb.append("Total time: " + (System.nanoTime() -
						// totalTime1) / 1000000 + " ms. \n");
			...
			try {
				future.get();
				// assertFalse("Script should return an cancellation exception",
				// true);
